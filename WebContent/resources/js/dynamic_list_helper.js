...
function DynamicListHelper( config ) {
	...
	console.log("dynamic helper called !!!");
    init();
    ...
    function prepFormForSubmit() {
    	...
    	console.log("coming in prepform submit after adding a row at the end.");
        ...
        for( var i in memberArray ) {
            ...
            console.log("Value of class name is" + className);
            var index = 0;
            $(className).each( function(){
            	//console.log($(this).attr('name',config.indexedPropertyName + "["+index+"]." + $.trim(memberArray[i])));
    ...
    // if row.length is 0 then row row = lastRow
	// if row.lenth is  not zero 
	
    function addRow() {
        var row = $(config.formId).find(config.rowClass+":last").clone(true);
        console.log(" after listening adding a row lets see what row var has");
        console.log(row);
        if( !row.length ){
        	console.log("in the check not equal to row.length");
            row  = lastRow;
        }
        
        console.log("Lets see what type of row.attr of id has" + typeof $(row).attr('id'))
        
        // still dont know what this does since for new row its coming undefined.
        if(typeof $(row).attr('id') !== 'undefined') {
            $(row).attr('id', $(row).attr('id')+'_1' );
            console.log($(row).attr('id', $(row).attr('id')+'_1' ));
        }
    ...
    function addAddRowListener() {
        $(config.addRowId).click( function(){
        	 console.log("Add Row button clicked from listner")
            addRow();
            return false;
        });
    }
